...
def _validate_depth(scene_id: str, depth: dict[str, Any]) -> dict[str, Any]:
    if not isinstance(depth, dict):
        raise ValueError(f"depth evidence must be an object for {scene_id}")
    edges = depth.get("edges", [])
    unknowns = depth.get("unknown_relations", [])
    if not isinstance(edges, list) or not isinstance(unknowns, list):
        raise ValueError(f"depth edges/unknown_relations must be lists for {scene_id}")
    ...
    for edge in edges:
        if not isinstance(edge, dict):
            raise ValueError(f"invalid depth edge for {scene_id}: {edge!r}")
        front = _nonempty_string(edge.get("front"), name=f"{scene_id}.depth.front")
        behind = _nonempty_string(edge.get("behind"), name=f"{scene_id}.depth.behind")
        if front == behind:
        ...
        _nonempty_string(edge.get("authority"), name=f"{scene_id}.depth.authority")

    seen_unknown_pairs: set[tuple[str, str]] = set()
    for rel in unknowns:
        if not isinstance(rel, dict):
            raise ValueError(f"invalid UNKNOWN depth relation for {scene_id}: {rel!r}")
        pair = rel.get("pair", [])
        if not isinstance(pair, list) or len(pair) != 2:
            raise ValueError(f"invalid UNKNOWN depth relation for {scene_id}: {rel}")
        a = _nonempty_string(pair[0], name=f"{scene_id}.depth.unknown_pair[0]")
        b = _nonempty_string(pair[1], name=f"{scene_id}.depth.unknown_pair[1]")
        if a == b:
            raise ValueError(f"invalid UNKNOWN depth relation for {scene_id}: self relation {a!r}")
        _nonempty_string(rel.get("reason"), name=f"{scene_id}.depth.unknown_reason")
        canonical_pair = tuple(sorted((a, b)))
        if canonical_pair in seen_unknown_pairs:
            raise ValueError(f"duplicate UNKNOWN depth relation for {scene_id}: {a}<->{b}")
        seen_unknown_pairs.add(canonical_pair)
...
def _validate_micro(data: dict[str, Any]) -> dict[str, Any]:
    cross = data.get("micro_motion_cross_golden")
    if not isinstance(cross, dict):
        raise ValueError("micro_motion_cross_golden must be an object")
    if _nonnegative_int(cross.get("candidate_count"), name="micro_motion_cross_golden.candidate_count") != 220:
    ...
    for scene_id, scene in scenes.items():
        if not isinstance(scene, dict) or not isinstance(scene.get("micro_motion"), dict):
            raise ValueError(f"micro-motion evidence must be an object for {scene_id}")
    ...
    for key, value in expected.items():
        observed = _nonnegative_int(cross.get(key), name=f"micro_motion_cross_golden.{key}")
        if observed != value:
...
def _validate_camera(scene_id: str, scene: dict[str, Any]) -> dict[str, Any]:
    if not isinstance(scene, dict) or not isinstance(scene.get("camera"), dict) or not isinstance(scene.get("flow_summary"), dict):
        raise ValueError(f"camera/flow evidence must be objects for {scene_id}")
    cam = scene["camera"]
    ...
    bg_vec = _finite_nonnegative(
        flow.get("background_abs_vector_median"),
        name=f"{scene_id}.flow_summary.background_abs_vector_median",
    )
    klass = _nonempty_string(cam.get("observable_class"), name=f"{scene_id}.camera.observable_class")
    global_state = _nonempty_string(cam.get("global_frame_state"), name=f"{scene_id}.camera.global_frame_state")
...
def compile_camera_depth(data: dict[str, Any]) -> dict[str, Any]:
    if not isinstance(data, dict):
        raise ValueError("W2 camera/depth evidence must be an object")
    ...
    scenes = data.get("scenes", {})
    if not isinstance(scenes, dict) or set(scenes) != SCENES:
        raise ValueError("W2 golden scene set drift")
    refs = [scene.get("source_ref", "") if isinstance(scene, dict) else "" for scene in scenes.values()]
    ...
    if not isinstance(evidence, dict):
        raise ValueError("full_evidence must be an object")
    for name in ("optical_flow_baseline", "micro_motion_corroboration", "w1_motion_kinematics"):
        item = evidence.get(name, {})
        if not isinstance(item, dict):
            raise ValueError(f"durable evidence pointer must be an object: {name}")
        _nonempty_string(item.get("drive_id"), name=f"full_evidence.{name}.drive_id")
        if not _hex64(item.get("sha256")):
            raise ValueError(f"durable evidence SHA-256 must be 64 lowercase hex characters: {name}")
