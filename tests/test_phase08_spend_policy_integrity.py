...
def test_valid_authorization_preserves_normalized_credit_value_and_initial_submit():
...
def test_retry_requires_bounded_estimate_and_respects_per_render_cap():
    missing = RenderIntent(
        intent_id="RND_X",
        content_id="CNT_X",
        profile_id="P_X",
        script_hash="a" * 64,
    ...
    with pytest.raises(ValueError, match="estimated_credits is required"):
        next_retry(missing, _policy())

    over_cap = RenderIntent(**{**missing.__dict__, "estimated_credits": 11.0})
    with pytest.raises(RuntimeError, match="per-render"):
        next_retry(over_cap, _policy())
...
def test_existing_record_must_be_typed_and_reconcilable():
    original = _authorize()
    failed = RenderIntent(**{**original.__dict__, "state": RenderState.FAILED_RETRYABLE})
    retried = next_retry(failed, _policy())
    assert can_submit(
        retried,
        {original.intent_id: {"state": "FAILED_RETRYABLE"}},
        policy=_policy(),
        spent_today=0.0,
        concurrent_renders=0,
    ) is False
