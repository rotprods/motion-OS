...
def test_audio_integrity_policy_tracks_master_presence():
    assert _plan(audio_path='master.wav')['audio_integrity_policy'] == 'post_render_master_audio_required'
    assert _plan(audio_path=None)['audio_integrity_policy'] == 'explicit_silence'
...
def test_post_render_wrapper_requires_intact_audio_contract():
    plan=_plan()
    plan['audio_contract']={'sample_rate':48000,'channels':True}
    with pytest.raises(ValueError,match='audio_contract channels'):
        verify_assembled_output_audio(plan,'out.mp4')
