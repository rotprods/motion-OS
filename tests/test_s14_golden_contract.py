...
def test_renderer_consumes_measured_track_and_registers_semantically():
 root=ROOT_TSX.read_text();spec=SPEC.read_text();track=TRACK.read_text();comp=COMP.read_text();assert "from './golden_s14'" in root;assert 'id={S14_SPEC.compositionId}' in root;assert 'id={S14_SPEC.overlayCompositionId}' in root;assert "sceneId:'S14_AUDIO_VISUAL_TEXTO'" in spec;assert 'MEASURED_VISIBLE_KEYFRAME_PROJECTION' in track;assert 'measuredS14(frame)' in comp
