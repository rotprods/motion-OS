...
MEASUREMENT_COLORS={
 ...
 'annotation_audio':(255,26,102),'annotation_visual':(109,255,0),'annotation_texto':(166,108,255),
}
...
def observe_measurement_color(image:Image.Image,expected:Box,target:tuple[int,int,int],pad:int=12,tolerance:int=18)->Box|None:
    """Measure one target-isolated renderer entity by unique QA color identity."""
...
def observe_named(image:Image.Image,expected:Box,name:str)->Box|None:
    return observe_measurement_color(image,expected,MEASUREMENT_COLORS[name])
...
def load_annotations(data:dict)->dict[str,dict[int,Box]]:
    if data.get('schema_version')!='motion-os.s14-annotation-track/v1':raise ValueError('unsupported S14 annotation-track schema')
    out={f'annotation_{s}':{} for s in ('audio','visual','texto')}
    for row in data['frames']:
        f=int(row['local_frame'])
        for state in ('audio','visual','texto'):
            raw=row['annotations'].get(state)
            if raw is None:continue
            b=raw['screen_bbox'];out[f'annotation_{state}'][f]=Box(float(b['x']),float(b['y']),float(b['width']),float(b['height']))
    return out
...
def qualify(overlay_dir:Path,measured:dict,annotations:dict|None=None)->dict:
    if measured.get('schema_version') not in {'motion-os.s14-measured-track/v1','motion-os.s14-measured-track/v2'}:raise ValueError('unsupported S14 measured-track schema')
    source=measured['source'];expected=load_expected(measured)
    if annotations is not None:expected.update(load_annotations(annotations))
    metrics={};per_frame={}
    for name,track in expected.items():
        ...
        for f,exp in sorted(track.items()):
            ...
            obs=observe_named(image,exp,name)
    ...
    card_names=['card_audio','card_visual','card_texto'];head_names=['heading_audio','heading_visual','heading_texto'];annotation_names=['annotation_audio','annotation_visual','annotation_texto']
    gates={
    ...
    if annotations is None:
        gates['annotation_geometry']='NOT_RUN'
        visible_pass=False
    else:
        gates['annotation_geometry']=all(metrics[n]['mean_bbox_iou'] is not None and metrics[n]['mean_bbox_iou']>=0.72 and metrics[n]['mean_centroid_error_px']<=8 for n in annotation_names)
        gates['annotation_timing']=all(metrics[n]['first_visible_error_frames'] is not None and abs(metrics[n]['first_visible_error_frames'])<=1 and abs(metrics[n]['last_visible_error_frames'])<=1 for n in annotation_names)
        visible_pass=all(v is True for v in gates.values())
    return {'schema_version':'motion-os.s14-source-bound-qualification/v2','scene_id':'S14_AUDIO_VISUAL_TEXTO','source':source,'measured_schema':measured['schema_version'],'measurement_mode':'TARGET_ISOLATED_UNIQUE_COLOR_RENDER','authority':'MEASURED_VISIBLE_OUTPUT_PARTIAL_QUALIFICATION','metrics':metrics,'gates':gates,'p0_p1_visible_state_gates_pass':visible_pass,'full_9d_fidelity_validated':False,'blocked_dimensions':['exact annotation vector-path topology/stroke pre-compression','exact heading font identity/source glyph outline','nested source-media pixel fidelity in structural mode','original AE graph/Graph Editor curves','isolated original SFX stems','full FX/color/depth/camera decomposition'],'per_frame':per_frame}

def main()->int:
    ap=argparse.ArgumentParser();ap.add_argument('--overlay-dir',type=Path,required=True);ap.add_argument('--measured',type=Path,required=True);ap.add_argument('--annotations',type=Path);ap.add_argument('--out',type=Path,required=True);a=ap.parse_args();m=json.loads(a.measured.read_text());anns=json.loads(a.annotations.read_text()) if a.annotations else None;out=qualify(a.overlay_dir,m,anns);a.out.parent.mkdir(parents=True,exist_ok=True);a.out.write_text(json.dumps(out,indent=2)+'\n');print(json.dumps({k:out[k] for k in ('authority','measurement_mode','gates','p0_p1_visible_state_gates_pass')},indent=2));return 0
