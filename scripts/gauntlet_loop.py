...
def evaluate_gauntlet(
    ...
    *,
    ...
    if latest.verifier_complete:
        ...
        # This pure evaluator can validate a receipt's structure and bindings,
        # but it cannot prove the verifier's identity/independence from caller data.
        # Therefore it must never mint VERIFIED authority on its own. A canonical
        # Event Fabric / clean-runner projection outside this function owns that promotion.
        return {
            "state": "VERIFIER_PASS_UNPROMOTED",
            "reason": latest.verifier_reason or "VERIFIER_COMPLETE",
            "attempts": len(parsed),
            "result_hash": latest.result_hash,
            "verifier_receipt": receipt,
            "promotion_authority": "NONE",
            "next_action": "bind verifier evidence through canonical external authority before VERIFIED projection",
        }
