...
def scan_secrets(path: Path, *, root: Path = ROOT) -> list[Finding]:
    ...
    try:
        ...
        return [Finding("TEXT_SOURCE_UNREADABLE", rel, 0, "Selected text could not be audited")]
    findings: list[Finding] = []
...
def iter_files(roots: Iterable[Path], *, root: Path = ROOT) -> tuple[Path, ...]:
    ...
    for item in roots:
        ...
        if path.is_symlink() or not path.resolve().is_relative_to(root) or not path.exists():
        ...
        for candidate in path.rglob("*"):
            ...
            if candidate.is_symlink():
                raise ValueError("symlink_scan_scope")
            ...
            files.add(candidate)
    if not files:
        raise ValueError("empty_scan_scope")
...
def scan_repository(roots: Iterable[Path], *, root: Path = ROOT) -> tuple[Finding, ...]:
    findings: list[Finding] = []
    try:
        files = iter_files(roots, root=root)
    except (OSError, ValueError):
        return (Finding("SCAN_SCOPE_INVALID", "<scope>", 0, "Scan scope is missing, empty, inaccessible or unsafe"),)
    for path in files:
