...
class SurfaceEvent:
    ...
    def __post_init__(self) -> None:
        ...
        if len(self.payload_hash) != 64:
            raise ValueError("payload_hash must be sha256")
...
def deduplicate_surface_events(events: Iterable[SurfaceEvent]) -> tuple[SurfaceEvent, ...]:
    """Deduplicate the same logical fact across transports.

    Same logical ID + same payload hash is one fact. Same logical ID + different
    payload hash is contradictory evidence and fails closed.
    """
    ...
    for item in events:
        ...
        # deterministic preferred representation; semantics are identical.
        if item.surface.value < existing.surface.value:
            chosen[item.logical_id] = item
    return tuple(sorted(chosen.values(), key=lambda x: x.logical_id))


def reconcile_github_lifecycle(
    projected: Mapping[str, str], live: Mapping[str, str]
) -> dict[str, str]:
    ...
    def compile(
        ...
        *,
        ...
        for index, event in enumerate(events):
            ...
            if event_session != identity.session_id:
            ...
            for parent in event.get("parent_event_ids", ()) or ():
        ...
        sorted_nodes = tuple(sorted(nodes.values(), key=lambda x: (x.node_type, x.node_id, x.properties_json)))
        sorted_edges = tuple(sorted(edges.values(), key=lambda x: (x.source, x.relation, x.target, x.properties_json)))
        payload = {
