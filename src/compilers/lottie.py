...
def validate_lottie_subset(doc: dict[str, Any]) -> LottieValidation:
    ...
    try:
        fps=float(doc.get("fr",0)); in_frame=float(doc.get("ip",0)); out_frame=float(doc.get("op",0)); width=int(doc.get("w",0)); height=int(doc.get("h",0))
    except (TypeError,ValueError): fps=in_frame=out_frame=0; width=height=0
    if fps<=0: unsupported.append("invalid_fps")
    if out_frame<=in_frame: unsupported.append("invalid_frame_range")
    ...
    asset_ids={asset.get("id") for asset in assets if isinstance(asset,dict) and asset.get("id")}
    indices=[]; names=[]
    for layer in layers:
        ...
        if not isinstance(index,int) or index<=0: unsupported.append("invalid_layer_index")
        ...
        if typ in {"image","precomp"}:
            ...
            elif ref_id not in asset_ids: unsupported.append(f"unresolved_asset_ref:{ref_id}")
...
def _compile_layer(element: dict[str, Any], *, index:int, in_frame:int, out_frame:int) -> dict[str, Any]:
    ...
    features=list(element.get("features",[])); unsupported_features=sorted(set(features)-SUPPORTED_FEATURES)
...
def compile_vector_subgraph_to_lottie(elements:list[dict[str,Any]], *, width:int=1080,height:int=1920,fps:int=30,in_frame:int=0,out_frame:int=300,markers:list[dict[str,Any]]|None=None,assets:list[dict[str,Any]]|None=None,fonts:dict[str,Any]|None=None) -> dict[str,Any]:
    if width<=0 or height<=0: raise ValueError("Lottie canvas dimensions must be positive")
    if fps<=0: raise ValueError("Lottie fps must be positive")
    if out_frame<=in_frame: raise ValueError("Lottie out_frame must be greater than in_frame")
