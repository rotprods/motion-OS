        ...
        for e in graph.edges:
            ...
            if child.kind=="Layer":
                ...
                for phase in ("entry","settle","exit"):
                    event=cd.get(phase)
                    if isinstance(event,dict): timeline.append({"id":f"{child.id}:{phase}","sceneId":scene.id,"target":child.id,"atMs":start+int(event.get("at_ms",0)),"durationMs":int(event.get("duration_ms",0)),"action":event.get("action",phase),"ease":event.get("ease","power3.out"),"channels":event.get("channels",{})})
...
def emit_hyperframes_project(spec: HyperFramesSpec) -> dict[str,str]:
    ...
    data=json.dumps(spec.to_dict(),indent=2,ensure_ascii=False); compact=json.dumps(spec.to_dict(),sort_keys=True,separators=(",",":"),ensure_ascii=False); duration_s=spec.duration_ms/1000
    js=f"""'use strict';
const spec={compact};
