...
_YUV_PIXEL_PREFIXES = ("yuv", "yuva", "nv12", "nv21", "p010", "p012", "p016", "yuyv", "uyvy")
...
def pixel_family_from_probe(artifact_id: str, probe: Mapping[str, Any]) -> str:
    """Classify the *encoded media* pixel family without inferring it from origin.

    A browser-originated render may already be encoded as YUV. That media must not
    be fed to zscale with RGB/GBR matrix coefficients merely because its authored
    source was browser/sRGB. Unknown pixel formats fail closed.
    """
    if not artifact_id.strip():
        raise ValueError("pixel family requires artifact_id")
    pix_fmt = str(probe.get("pix_fmt") or "").strip().lower()
    if not pix_fmt:
        raise ValueError(f"unqualified_pixel_format:{artifact_id}:missing")
    if pix_fmt in _RGB_PIXEL_FORMATS or pix_fmt.startswith("gbr"):
        return "rgb"
    if pix_fmt.startswith(_YUV_PIXEL_PREFIXES):
        return "yuv"
    raise ValueError(f"unqualified_pixel_format:{artifact_id}:{pix_fmt}")


def color_profile_from_probe(
    artifact_id: str,
...
def _canonical_payload(*, target, bindings, backend, tone_map_policy, authority):
    return {
        ...
        "bindings": [
            ...
            for binding in sorted(bindings, key=lambda value: value.artifact_id)
        ],
        "backend": backend,
...
def build_color_normalization_plan(
    ...
    source_profiles,
    *,
    alpha_artifacts=frozenset(),
    target=BT709_SDR_LIMITED,
    backend="zscale",
    tone_map_policy="reject_hdr",
):
    if not artifact_ids:
    ...
    if extra:
        raise ValueError("unbound_color_profiles:" + ",".join(sorted(extra)))
    bindings = []
    for artifact_id in artifact_ids:
        ...
        binding = ArtifactColorBinding(
            artifact_id,
            profile,
            artifact_id in alpha_artifacts,
        )
        binding.validate()
    ...
    return ColorNormalizationPlan(
        target,
        tuple(sorted(bindings, key=lambda value: value.artifact_id)),
        backend,
        tone_map_policy,
        authority,
        plan_hash,
...
def ffmpeg_output_color_args(target: ColorProfile = BT709_SDR_LIMITED) -> list[str]:
    ...
    return [
        ...
        target.matrix,
        "-color_range",
        "tv" if target.range == "limited" else "pc",
    ]
...
def validate_color_plan(plan, artifact_ids):
    ...
    try:
        ...
        [binding.validate() for binding in plan.bindings]
