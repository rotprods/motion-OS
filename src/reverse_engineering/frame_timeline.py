...
def _finite_number(raw: object, *, name: str, nonnegative: bool = False) -> float:
...
def _total_frames(pack: Mapping[str, Any]) -> int:
    ...
    total = _frame_index(raw, name="decoded_frame_count")
    if total <= 0:
...
def _fps(pack: Mapping[str, Any]) -> float:
    value = _finite_number(pack.get("video_meta", {}).get("fps", 0.0), name="fps")
    if value <= 0:
...
def compile_frame_timeline(
    ...
    if isinstance(motion_stats, Mapping) and motion_stats.get("available"):
        ...
        for track in tracks:
            ...
            motion_by_frame[frame] = {
                "from_frame": frame,
                "to_frame": to_frame,
                "global_dx": _finite_number(track.get("global_dx", 0.0), name="motion.global_dx"),
                "global_dy": _finite_number(track.get("global_dy", 0.0), name="motion.global_dy"),
                "global_magnitude": _finite_number(track.get("global_magnitude", 0.0), name="motion.global_magnitude", nonnegative=True),
                "local_residual_median": _finite_number(track.get("local_residual_median", 0.0), name="motion.local_residual_median", nonnegative=True),
                "motion_median": _finite_number(track.get("motion_median", 0.0), name="motion.motion_median", nonnegative=True),
                "camera_likelihood": _finite_number(track.get("camera_likelihood", 0.0), name="motion.camera_likelihood", nonnegative=True),
                "authority": "measured",
    ...
    for item in feature_pack.get("ocr", []):
        ...
        if confidence is not None:
            confidence = _finite_number(confidence, name="ocr.confidence", nonnegative=True)
            if confidence > 1.0:
    ...
    if isinstance(audio, Mapping) and audio.get("available"):
        ...
        for index, onset in enumerate(onsets):
            onset_value = _finite_number(onset, name="audio.onset_ms", nonnegative=True)
        ...
        for shot in shots:
            ...
            for step in shot.get("micro_choreography", []):
                ...
                if "at_frame" in step:
                    ...
                    at_ms = _finite_number(step.get("at_ms", 0), name="micro_choreography.at_ms", nonnegative=True)
                    frame = int(round(at_ms * fps / 1000.0))
                if 0 <= frame < total_frames:
                    normalized = dict(step)
                    # MotionStyle choreography is derived/inferred. Never preserve an
                    # upstream stronger authority label as if this timeline measured it.
            ...
            if transition:
                ...
                raw_at_ms = transition.get("at_ms_global", shot.get("end_ms", 0))
                at_ms = int(round(_finite_number(raw_at_ms, name="transition.at_ms", nonnegative=True)))
