# MOTION.OS V2 — Graph Projections

Authority: `IMPLEMENTED_PENDING_REVIEW`
Canonical machine graph: `graph/v2/motion_os_v2_hypergraph.json`

Every diagram below is a **derived projection over shared IDs**, never independent write authority.
...
```mermaid
flowchart LR
  Project --> Agent --> Session --> Workstream
  Session --> Claim --> Scope
  Session --> Checkpoint
...
flowchart LR
  ...
  Source --> Claim
  Claim --> Evidence
  Claim --> Assumption --> Risk
  Claim --> Decision --> Contract --> Module
...
flowchart LR
  ...
  NodeDrift[Non-reproducible Node] --> RendererDrift --> BadPromotion
...
flowchart LR
  ...
  Invariant --> Security
  Invariant --> Recovery
  HistoricalBug --> RegressionTest --> FailureFamily --> Property
```
...
flowchart LR
  ...
  EvidenceEnvelope --> Verification --> AuthorityState --> ReleaseDecision
```
...
flowchart LR
  ...
  MasterAudio --> Assembly --> MasterMedia
...
flowchart LR
  ...
  NewContextPack --> ReconcileRecovery --> NextSafeAction
...
flowchart TB
  ...
  ContentIntelligence --> StudioRuntime --> RendererFabric --> TemporalQA --> EmpiricalQualification
  AgentRuntime --> EventFabric --> AuthorityKernel
  EvidencePlane --> AuthorityKernel --> DerivedCOS
```

## 17. Historical Graph
```mermaid
flowchart LR
  Bootstrap --> CreativeConvergence
...
## 19. Product / Creative Quality Graph — COS L17
```mermaid
flowchart LR
...
## 20. Empirical Learning Graph — COS L18
```mermaid
flowchart LR
  Publication --> Observation --> PerformanceEvidence --> LearningCandidate --> ControlledExperiment
  ControlledExperiment -->|supported| PromotedRule --> FutureStrategy --> Publication
  ControlledExperiment -->|not supported| RejectedRule
```

## 21. Release Authority Graph — COS L19
```mermaid
flowchart LR
  MainFresh --> Gate
  EventFresh --> Gate
...
Every projection is invalidated when any declared source revision/watermark changes. Similarity, GraphRAG, dashboards and traversal may discover relationships but cannot invent authority. A projection that cannot be deterministically rebuilt from its declared source set is a V2 defect.