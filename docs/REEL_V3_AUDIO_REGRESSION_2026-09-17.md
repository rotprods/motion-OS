...
## Real incident
A Reel edit path produced an apparently successful visual deliverable while the user-observed audio was lost. This is a P0 delivery defect: a speech master must never qualify from visual checks alone.
...
2. makes post-render audio integrity an explicit plan policy whenever a master audio path exists;
3. probes the final encoded artifact, not merely the intended command;
...
- present-but-silent audio at declared speech windows;
